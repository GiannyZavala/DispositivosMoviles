...
/**
 * Created by GiannyEduardo on 28/05/17.
 */
...
public class ConexionWeb extends AsyncTask<URL,String,String> {
    ...
    protected void onPostExecute(String resp){
        dialogo.dismiss();
        if(resp.startsWith("408")){

        }
        if(resp.startsWith("099")){
            String[] resultados = resp.split(",");
            puntero.resultado.setText("El nombre es: "+resultados[1]+"\n"+"domicilio: "+resultados[2]+"\n"+"Carrera: "+resultados[3]+"\n"+
            "Promedio: "+resultados[4]);

        }else {
            puntero.mostrarMensajeEnAlert(resp);
        }
    }
}
