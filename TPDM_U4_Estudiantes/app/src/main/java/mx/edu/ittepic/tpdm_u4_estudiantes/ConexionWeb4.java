...
public class ConexionWeb4 extends AsyncTask<URL,String,String> {

    List<String[]> variables;
    Main3Activity puntero;
    ProgressDialog dialogo;

    public ConexionWeb4(Main3Activity p){
        puntero=p;
    ...
    protected void onPostExecute(String resp) {
        dialogo.dismiss();
        if(resp.startsWith("099")){
            puntero.aplicarfalta.setEnabled(true);
            puntero.aplicar.setEnabled(true);
            puntero.noaplicar.setEnabled(true);
        }

        String[] resultado= resp.split(",");
        puntero.resultadobusqueda.setText("Nombre: "+resultado[0]+"\nDomicilio: "+resultado[1]);
    }
}
